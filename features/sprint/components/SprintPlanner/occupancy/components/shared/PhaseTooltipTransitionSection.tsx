'use client';

import type { Developer } from '@/types';
import type { ChangelogEntry } from '@/types/tracker';

import { Avatar } from '@/components/Avatar';
import { StatusTag } from '@/components/StatusTag';
import { getInitials } from '@/utils/displayUtils';

interface PhaseTooltipTransitionItem {
  entry: ChangelogEntry;
  fromStatusKey: string | null;
  fromStatusName: string | null;
  timestamp: string;
  toStatusKey: string;
  toStatusName: string;
}

interface PhaseTooltipTransitionSectionProps {
  contributingLines: string[];
  developerMap: Map<string, Developer>;
  transitions: PhaseTooltipTransitionItem[];
  formatDateTime: (timestamp: string) => string;
}

export function PhaseTooltipTransitionSection({
  contributingLines,
  developerMap,
  formatDateTime,
  transitions,
}: PhaseTooltipTransitionSectionProps) {
  if (transitions.length > 0) {
    return (
      <div className="space-y-3">
        {transitions.map((transition, idx) => {
          const author = transition.entry.createdBy;
          const authorId = author?.id;
          const developer = authorId ? developerMap.get(authorId) : undefined;
          const authorName = developer?.name || author?.display || 'Неизвестно';
          const avatarUrl = developer?.avatarUrl;

          return (
            <div key={idx} className="flex items-start gap-2">
              <Avatar avatarUrl={avatarUrl} initials={getInitials(authorName)} size="lg" />
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1">
                  <span className="text-sm font-medium text-gray-900 dark:text-gray-100">
                    {authorName}
                  </span>
                  <span className="text-xs text-gray-500 dark:text-gray-400 ml-auto">
                    {formatDateTime(transition.timestamp)}
                  </span>
                </div>
                <div className="flex items-center gap-2">
                  {transition.fromStatusKey ? (
                    <>
                      <StatusTag
                        label={transition.fromStatusName ?? undefined}
                        status={transition.fromStatusKey}
                      />
                      <span className="text-gray-400 dark:text-gray-500">→</span>
                    </>
                  ) : null}
                  <StatusTag label={transition.toStatusName} status={transition.toStatusKey} />
                </div>
              </div>
            </div>
          );
        })}
      </div>
    );
  }

  if (contributingLines.length === 0) {
    return (
      <div className="text-sm text-gray-500 dark:text-gray-400">
        Нет информации о переходах
      </div>
    );
  }

  return null;
}
